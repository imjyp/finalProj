...
(function ($, window) {
        ...
        toggleButton: function () {
            $('.toggle').find('>button').each(function () {
                ...
                $(this).on('click', function (evt) {
                    var s, n;
                    evt.preventDefault();
                    if (t.attr('aria-selected') == 'true' || b.hasClass('open')) {
                        t.attr({
                            'aria-selected': false,
                            'aria-expanded': false
                        });
        ...
        tabToggle: function () {
            $('.tabMn,.subtabMn').each(function () {
                $(this).on('click', 'li>a', function (evt) {
                    ...
                    $(this).parent().siblings().find('.hide').remove();
                    $(this).prepend('<span class="hide">선택됨</span>');
                    var ul = _this.closest('ul , ol');
                    if (ul.hasClass('foodTab')) {
                        var name = $(this).parent().attr('class').replace(' on', '');
                    ...
                    if (ul.hasClass('tabMn')) {
                        ...
                        var _href = $(_this.attr('href')).find('>ul>li>a[aria-selected=true],>ol>li>a[aria-selected=true],>ul>li>a.on,>ol>li>a.on').attr('href');
                        $(_href).show();
                ...
                if ($active.hasClass('video')) {
                    ...
                    $btn.off().on('click', function (e) {
                        ...
                        if (!$btn.hasClass('off')) {
                            ...
                            $btn.addClass('off').text('광고 동영상 소리 켜기');
                        } else {
                            video.prop('muted', false);
                            $btn.removeClass('off').text('광고 동영상 소리 끄기');
                        }
                    });
    ...
    $(function () {
        ...
        // 2023-01-18 추가
        $('.basicSwiper .ctrl').each(function () {
            $(this).removeAttr('tabindex role aria-label');
        })
    });
