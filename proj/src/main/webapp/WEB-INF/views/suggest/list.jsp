	...
	<div class="card-header">
		...
		<div class="col-md-6 d-flex align-items-center">
	          <nav aria-label="breadcrumb" class="ms-3">
	            ...
	            <li class="breadcrumb-item active"><a href="/suggest/list">건의사항</a></li>
	         </ol>
          </nav>
        </div>
	</div>	
				...
				<div
					...
					<input type="date" style="width:130px">
						...
						<sec:authorize access="isAuthenticated()"><!-- 로그인 했다면 보여라 -->
								<button onclick="location.href='/suggest/create'" class="btn btn-dark rounded-pill" style="float: right;">등록</button>
						</sec:authorize>
						</div>
					</div>
					<div class="tableType01" style="width:100% !important">
						<table class="board" id="table1">
							...
							<tbody id="tby">
<%-- 								<c:forEach var="suggestVO" items="${articlePage.content}"> --%>
<!-- 									<tr> -->
<%-- 										<td>${suggestVO.rnum}</td> --%>
<%-- 										<td><a href ="/suggest/detail?suggestBoardNo=${suggestVO.suggestBoardNo}&currentPage=${currentPage}">${suggestVO.suggestTitle}</a></td> --%>
<%-- 										<td>${suggestVO.userNm}</td> --%>
<%-- 										<td>${suggestVO.suggestDate}</td> --%>
<!-- 									</tr> -->
<%-- 								</c:forEach> --%>
							</tbody>
						</table>
			...
			<%-- <tbody id="tby">
				<c:forEach var="suggestVO" items="${articlePage.content}" varStatus="stat" >
              <tr>
	...
	function getList(currentPage, keyword){
		...
		$.ajax({
			...
			success:function(result){
				...
				let str="";
				$.each(result.content,function(idx,suggestVO){
					str+=`<tr>
						 <td>\${suggestVO.rnum}</td>
		                  <td style="text-align: left;"><a href ="/suggest/detail?suggestBoardNo=\${suggestVO.suggestBoardNo}&currentPage=\${currentPage}">\${suggestVO.suggestTitle}</a></td>
		                  <td>\${suggestVO.userNm}</td>
		                  <td>\${suggestVO.suggestDate}</td>
		                  </tr>`;
				});
