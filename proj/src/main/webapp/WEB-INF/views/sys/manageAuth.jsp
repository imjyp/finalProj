 ...
 <div id="main">
    ...
    <div class="card-content">
        <div class="card-body align-items-center justify-content-between" style="display: flex; margin-bottom: 20px;">
            <div class="dataTable-top" style="justify-content: right; display: flex; width: 100%; margin-left:32px;">
        ...
        <div class="tableType01">
            ...
            <colgroup>
                ...
                <thead>
                    <tr>
                        <th class="noline">번호</th>
                        <th class="noline">아이디</th>
                        <th class="noline">회원 이름</th>
                        <th class="noline">소속</th>
                        <th class="noline">부서/가맹점명</th>
                        <th class="noline">직책</th>
                        <th class="noline">회원상태</th>
                        <th class="noline">권한명</th>
                        <th class="noline">권한부여일</th>
                        <th class="noline">인증여부</th>
                        <th class="noline">권한변경</th>
                    </tr>
                </thead>
                <tbody id="tby">
                    <c:forEach var="userVO" items="${articlePage.content}" varStatus="stat">
                        <tr>
                            ...
                            <td style="text-align:center;">${userVO.rnum}</td>
                            <td style="text-align:center;">${userVO.userNo}</td>
                            ...
                            <td style="text-align:center;">${userVO.positionNm}</td>
                            <td style="text-align:center;">
                                <c:choose>
                                    <c:when test="${userVO.userStatus == '1'}">
                                        <span class="badge bg-primary" style="display: inline-block;">정상</span>
                                    </c:when>
                                    <c:when test="${userVO.userStatus == '2'}">
                                        <span class="badge bg-secondary" style="display: inline-block;">탈퇴</span>
                                    </c:when>
                                    <c:otherwise>기타</c:otherwise>
                                </c:choose>
                            </td>
                            <td style="text-align:center;">
                                <c:choose>
                                    <c:when test="${userVO.authName == 'ROLE_GY'}">
                                        <span class="badge bg-light-info" style="display:inline-grid;justify-content: center;align-items: center;">경영</span>
                                    </c:when>
                                    <c:when test="${userVO.authName == 'ROLE_IS'}">
                                        <span class="badge bg-light-info" style="display:inline-grid;justify-content: center;align-items: center;">인사/행정</span>
                                    </c:when>
                                    <c:when test="${userVO.authName == 'ROLE_GH'}">
                                        <span class="badge bg-light-info" style="display:inline-grid;justify-content: center;align-items: center;">전략/기획</span>
                                    </c:when>
                                    <c:when test="${userVO.authName == 'ROLE_JJ'}">
                                        <span class="badge bg-light-info" style="display:inline-grid;justify-content: center;align-items: center;">재정/회계</span>
                                    </c:when>
                                    <c:when test="${userVO.authName == 'ROLE_MR'}">
                                        <span class="badge bg-light-info" style="display:inline-grid;justify-content: center;align-items: center;">물류</span>
                                    </c:when>
                                    <c:when test="${userVO.authName == 'ROLE_GMJ'}">
                                        <span class="badge bg-light-warning" style="display:inline-grid;justify-content: center;align-items: center;">가맹점주</span>
                                    </c:when>
                                    <c:when test="${userVO.authName == 'ROLE_ALBA'}">
                                        <span class="badge bg-light-secondary" style="display:inline-grid;justify-content: center;align-items: center;">아르바이트</span>
                                    </c:when>
                                    <c:when test="${userVO.authName == 'ROLE_MANAGER'}">
                                        <span class="badge bg-light-secondary" style="display:inline-grid;justify-content: center;align-items: center;">매니저</span>
                                    </c:when>
                                    <c:when test="${userVO.authName == 'ROLE_SYS'}">
                                        <span class="badge bg-light-primary" style="display:inline-grid;justify-content: center;align-items: center;">시스템관리자</span>
                                    </c:when>
                                    <c:when test="${userVO.authName == 'ROLE_CEO'}">
                                        <span class="badge bg-light-primary" style="display:inline-grid;justify-content: center;align-items: center;">CEO</span>
                                    </c:when>
                                    <c:otherwise>
                                        <span class="badge bg-light-danger" style="display:inline-grid;justify-content: center;align-items: center;">권한 대기</span>
                                    </c:otherwise>
                                </c:choose>
                            </td>
                            <td style="text-align:center;">
                                ...
                                <form id="profileForm" method="post">
                                    <div class="row">
                                        <div class="col-12 col-lg-6">
                                            ...
                                            <div class="form-group">
                                                <label for="dept" class="form-label">부서/가맹점명</label>
...
<!--                     <div class="d-flex"> -->
                        <button type="button" class="btn btn-warning ms-1" style="display:none" id="ok" data-user-no="${userVO.userNo}">저장</button>
                        <button type="button" class="btn btn-warning ms-1" id="edit">수정</button>
                        <button type="button" class="btn btn-dark ms-1" id="exit" style="display:none">취소</button>
                        <button type="button" class="btn btn-dark ms-1" id="out" data-user-no="${userVO.userNo}">탈퇴</button>
<!--                     </div> -->
                </div>
            </div>
        </div>
  ...
  <div class="modal-dialog modal-dialog-centered modal-dialog-centered modal-dialog-scrollable">
      <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="modal-footer">
                <button type="button" id="saveAuthChanges" class="btn btn-warning">변경</button>
    ...
    $(document).on("click", ".userDetail", function (e) {
        ...
        $.ajax({
            ...
            success: function (response) {
            	...
            	// 가맹점이면 가맹점명, 본사면 부서명 출력
                if (userVO.userCode == 2 || userVO.userCode == 4) {
                    $("#dept").val(userVO.storeNm); // 가맹점명
                    $("#detailDeptNm").text(userVO.storeNm);
                } else {
                    $("#dept").val(userVO.deptNm); // 부서명
                    $("#detailDeptNm").text(userVO.deptNm);
                }
                console.log("부서/가맹점명 : " + (userVO.userCode == 2 || userVO.userCode == 4 ? userVO.storeNm : userVO.deptNm));
