	...
	<div class="card-header">
		...
		<div class="col-md-6 d-flex align-items-center">
	          <nav aria-label="breadcrumb" class="ms-3">
	            ...
	            <li class="breadcrumb-item active"><a href="/sanction/pending">결재대기함</a></li>
	         </ol>
          </nav>
        </div>
	</div>
	...
	function getList(currentPage, keyword){
		...
		$.ajax({
			...
			success:function(result){
				...
				let str="";
				$.each(result.content,function(idx,SanctionVO){
					
				//console.log("체킁: ",idx,sanctionDocVO);
					str+=`<tr>
						 <td>\${SanctionVO.rnum}</td>
						 <td>\${SanctionVO.docTypeNm}</td>
		                  <td style="text-align: left;"><a href="/sanction/detail?docNo=\${SanctionVO.docNo}&currentPage=\${currentPage}">\${SanctionVO.docTitle}</a></td>
		                  <td>\${SanctionVO.deptNm}</td>
		                  <td>\${SanctionVO.userNm}</td>
		                  <td>\${SanctionVO.docCreateDate}</td>													
		                  </tr>`;
				});
